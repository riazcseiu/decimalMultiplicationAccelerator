...
static void decFiniteMultiply(bcdnum *num, uByte *bcdacc,
 ...
 //-----------------------------------------------------
  ...
  #endif
 /*
  // testing bor base billion without converting
 uByte   *msdq, *lsdq;
  int a = 5, b = 6;
  msdq = &a;
  lsdq = &b;
...
//---------------------------------------------------------------
	  ...
	  //bellow code extract coefficeint in base billion form decfloat format 
...
//to test method 4 we need to modify original function 	decFiniteMultiply	 
	  int methodnumber =4;        //4 = Method-1 : BCD conversion Method-1
							     ...
							     // 3 = base billion addition New metod comparizon
								// 6 Method-5 all PPG in SW 
                                 //  7 Method -6 base thousand additon with ppg software new papear method-8
		 // methodnumber = 1;//2 ;//3;
...
#define MULSHIFTB 32
       ...
       sourlo = DFWORD(dfl, 1);                          \
              ...
              sourhi = DFWORD(dfl, 0);                            \
              (bufl)[1] = DPD2BCD8[((sourhi << 2) | (sourlo >> 30)) & 0x3ff] \
              + DPD2BCD8[(sourhi >> 7) & 0x3ff]                    \
...
for (int i=0,ii=0; ii <36 ; i+i+1,ii=ii+4)


pp[i]=  MID(sourlo,ii,ii+4);
