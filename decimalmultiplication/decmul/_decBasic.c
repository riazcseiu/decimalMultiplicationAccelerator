 ...
 //-----------------------------------------------------
  ...
  for (ub=bcdacc;; pa--, ub+=9) {        //uByte  *ub;//
    if (*pa!=0) {                       // split(s) needed
      ...
      UBFROMUI(ub,   UBTOUI(&BIN2BCD8[top*4]));
      UBFROMUI(ub+3, UBTOUI(&BIN2BCD8[mid*4]));
      UBFROMUI(ub+6, UBTOUI(&BIN2BCD	for (int mohon =0 ; mohon <10; mohon++)
		{
		aaa = read_cycles();
...
int methodnumber = 1;   //1 = calculating 1X-9X in base billion
							      //2 = using base thousand addition
							     // 3 = base billion addition
		 // methodnumber = 1;//2 ;//3;//4 new instraction riaz and riazshift
		  ...
		  case 1:
		  {
			  ...
			  for (int precomputei = 2; precomputei < 8; precomputei++) // 1-9X need to comment out BCD conversion part
			  {
...
#define MULSHIFTB 32
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////
         ...
         int PPaddshift(int pp1, int pp2)
	 {
	    ...
	    //multiplicand multiple
	     asm volatile
			  (
			        "add %[z], %[x], %[y]\n\t" //new instruction 
			        : [z] "=r" (pp)//output (rd) , here "=r" overwritten variable
				: [x] "r" (pp1), [y] "r" (pp2) //input, "r" regisger operend , x,y=rs1,rs2
			  );  
					   
	        // printf(" pp1 = %d  pp2 = %d riaz = %d \n",pp1,pp2,pp );
		 return pp;
        } 
...
//	 printf("decimal catch srart\n");       
//        unsigned long X = 0b0011000001001001001101001000001100001000000001100101011101110000;
...
//        unsigned long Y =  0b0011000001001001001101001000001100001000000001100101011101110000;
// 0b0011100110000011100001000100001110000100100010010001000100010111;//0b0011000001001001001101001000001100001000000001100101011101110000;
...
//	unsigned long BCDresult = accum_BCDadd(X, Y);
  ...
  //    printf("decimal rrr catch end\n %ld",BCDresult );
            

         // end = read_cycles();
	 // printf("Took %lu  method 3 cycles\n", end - start);
...
break;
		  case 2:
		  {
			  ...
			  GETCOEFFTHOU(dfl, tac);           // least-significant goes to [0]
												//GETCOEFFTHOU(dfr, tacr);
			  GETCOEFFTHOU(dfr, tacr);           // least-significant goes to [0]
			  ...
			  for (int precomputei = 2; precomputei < 10; precomputei++) // 1-9X need to comment out BCD conversion part
			  {
				  ...
				  (tac)[0] += tacr[0];
				  if (tac[0] > 999) { tac[0] -= 1000; tac[1]++; }
				  (tac)[1] += tacr[1];
				  if (tac[1] > 999) { tac[1] -= 1000; tac[2]++; }
...
(tac)[2] += tacr[2];
				  if (tac[2] > 999) { tac[2] -= 1000; tac[3]++; }
				  //sourhi = DFWORD(dfl, 0);
				  (tac)[3] += tacr[3];
				  if (tac[3] > 999) { tac[3] -= 1000; tac[4]++; }
				  (tac)[4] += tacr[4];
				  if (tac[4] > 999) { tac[4] -= 1000; tac[5]++; }
				  (tac)[5] += tacr[5];
				  /////////////////////////
			  }
			  }
		  ...
		  case 3:
		  {
			  ...
			  {
				  (bufl)[0] += bufl[0];
				  if (bufl[0] > 999999999) { bufl[0] -= 1000000000; bufl[1]++; }
				  (bufl)[1] += bufl[1];
		 ...
		 int c;
	 	 ...
	 	 int BCDadd(int a, int b) // add coefficient to generate Multiplicand multiple
	         {	   
			  ...
			  (  
			        ...
			        "add  %[z], %[x], %[y]\n\t" //new instruction 
			        : [z] "=r" (c)//output (rd) , here "=r" overwritten variable
				: [x] "r" (a), [y] "r" (b) //input, "r" regisger operend , x,y=rs1,rs2
			     
			  );  
                  

					   
	        // printf(" a = %d  b = %d riaz = %d \n",a,b,c );
		 return c;
	}
         ...
         int PPaddshift(int pp1, int pp2)
	 {
	    ...
	    //multiplicand multiple
	    int i;   
	    int mm[9];//mm = multiplicand multiple
	    mm[0]=x;
	    for(i=0; i<8; i++){
		   // printf("%d",i);
		    mm[i+1] = BCDadd(mm[i],mm[0]);
                     BCDadd(mm[i],mm[0]);
	    ...
	    //result = PPaddshift(1,2);

	    // printf("restult is %",result);	     
